...
- `sessions/copilot.rs`：GitHub Copilot `events.jsonl` 事件流与相邻 `workspace.yaml` 元数据的只读解析；toolRequests 与 execution_start 去重，execution_complete 经 toolCallId 映射回工具名。
- `sessions/hermes.rs`：Hermes Agent `state.db` 聚合库（主库与 `profiles/*` 命名库）的只读聚合与按需详情解析；按现存列自适应查询，过滤 active=0 压缩归档行并排除委托子代理会话。
- `sessions/vscode_copilot.rs`：VS Code Copilot Chat 会话文件（平铺 `.json` 与 1.109 起 `.jsonl` 追加日志）的只读解析；`.jsonl` 逐条回放 set/push/delete，同 id 并存时取代 `.json`；连续正文片段与内联文件引用合并，空占位会话不进列表。
...
Pi、Kimi Code CLI、OpenCode、Kilo 和 ZCode 当前按只读来源接入。它们支持 AllSessions 工作区内的收藏、标签、备注、归档和软移除，但不生成 `_delete_ref`，后端也拒绝永久删除其原始会话或消息。Cursor、Devin、GitHub Copilot、Hermes Agent 与 VS Code Copilot Chat 同为只读来源。
...
- [GitHub Copilot](./sources/copilot.md)
- [Hermes Agent](./sources/hermes.md)
- [VS Code Copilot Chat](./sources/vscode-copilot.md)
