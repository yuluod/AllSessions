...
export function releaseFileName({ platform, arch, version }) {
  if (platform === "windows")
    return `AllSessions-${version}-windows-${arch}-setup.exe`;
...
export function updaterFileName({ platform, arch, version }) {
  if (platform === "windows")
    return releaseFileName({ platform, arch, version });
  if (platform === "mac")
    return `AllSessions-${version}-mac-${arch}.app.tar.gz`;
...
async function findFiles(root, extension) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory())
      matches.push(...(await findFiles(fullPath, extension)));
...
export async function collectArtifact({
  ...
  const extension =
    platform === "windows" ? ".exe" : platform === "mac" ? ".dmg" : ".deb";
  ...
  if (matches.length !== 1)
    throw new Error(
      `期望找到 1 个 ${extension} 安装包，实际找到 ${matches.length} 个`
    );
  const outputDir = path.resolve(projectRoot, output);
  await mkdir(outputDir, { recursive: true });

  const stableDestination = path.join(
    outputDir,
    releaseFileName({ platform, arch, version: packageJson.version })
  );
  const updaterDestination = path.join(
    outputDir,
    updaterFileName({ platform, arch, version: packageJson.version })
  );
  const updaterMatches =
    platform === "mac" ? await findFiles(bundleRoot, ".app.tar.gz") : matches;
  if (updaterMatches.length !== 1)
    throw new Error(
      `期望找到 1 个 ${platform} 更新包，实际找到 ${updaterMatches.length} 个`
    );
  ...
  await Promise.all(
    [...copies].map(([destination, source]) => copyFile(source, destination))
  );
  return [...copies.keys()];
}
