...
export function extractReleaseNotes(changelog, tagName) {
  ...
  const headingPattern = new RegExp(`^## \\[${escapedVersion}\\](?:\\s+-\\s+\\d{4}-\\d{2}-\\d{2})?\\s*$`, "m");
  ...
  const notes = (nextHeading ? remaining.slice(0, nextHeading.index) : remaining).trim();
  if (!notes) {
...
async function main() {
  const [tagName, input = "CHANGELOG.md", output = "release-notes.md"] = process.argv.slice(2);
  if (!tagName) {
    throw new Error("Usage: node scripts/extract-release-notes.mjs <tag> [input] [output]");
  }
  const changelog = await readFile(path.resolve(input), "utf8");
  await writeFile(path.resolve(output), extractReleaseNotes(changelog, tagName), "utf8");
}
