...
fn open_database(path: &Path) -> Result<Connection, String> {
    ...
    .map_err(|error| format!("无法只读打开 ZCode 数据库（{}）：{error}", path.display()))?;
    connection
        .busy_timeout(Duration::from_secs(2))
        .map_err(|error| format!("无法配置 ZCode 数据库读取超时：{error}"))?;
    Ok(connection)
}

fn parse_json(text: String, record: &str, id: &str) -> Result<Value, String> {
    serde_json::from_str(&text)
        .map_err(|error| format!("ZCode {record} {id} 的 JSON 无效：{error}"))
}
...
fn session_row(row: &Row<'_>) -> rusqlite::Result<SessionRow> {
    Ok(SessionRow {
        ...
        directory: row.get(2)?,
        title: row.get(3)?,
        task_type: row.get::<_, Option<String>>(4)?.unwrap_or_default(),
...
fn load_sessions(connection: &Connection) -> Result<Vec<SessionRow>, String> {
    ...
    let rows = statement
        ...
        .map_err(|error| format!("无法查询 ZCode 会话：{error}"))?;
    rows.collect::<Result<Vec<_>, _>>()
        .map_err(|error| format!("无法读取 ZCode 会话：{error}"))
}
...
fn load_session(connection: &Connection, session_id: &str) -> Result<SessionRow, String> {
    connection
        ...
        .map_err(|error| format!("无法读取 ZCode 会话 {session_id}：{error}"))
}
...
fn visit_parts(
    ...
    .map_err(|error| format!("无法查询 ZCode 消息片段：{error}"))?;
    while let Some(row) = rows
        .next()
        .map_err(|error| format!("无法读取 ZCode 消息片段：{error}"))?
    {
        let id = row
            .get::<_, String>(0)
            .map_err(|error| format!("无法读取 ZCode 片段 ID：{error}"))?;
        let message_id = row
            .get::<_, String>(1)
            .map_err(|error| format!("无法读取 ZCode 消息 ID：{error}"))?;
        let part = PartRow {
            id: id.clone(),
            message_id: message_id.clone(),
            session_id: row
                .get(2)
                .map_err(|error| format!("无法读取 ZCode 会话 ID：{error}"))?,
            time_created: row
                .get(3)
                .map_err(|error| format!("无法读取 ZCode 片段时间：{error}"))?,
            data: parse_json(
                row.get::<_, String>(4)
                    .map_err(|error| format!("无法读取 ZCode 片段数据：{error}"))?,
                "片段",
                &id,
            )?,
        };
        let message = MessageRow {
            ...
            time_created: row
                .get(6)
                .map_err(|error| format!("无法读取 ZCode 消息时间：{error}"))?,
            data: parse_json(
                row.get::<_, String>(7)
                    .map_err(|error| format!("无法读取 ZCode 消息数据：{error}"))?,
                "消息",
                &message_id,
            )?,
        };
...
fn validate_tables(connection: &Connection) -> Result<(), String> {
    for table in ["session", "message", "part"] {
        ...
        if count != 1 {
            return Err(format!("ZCode 数据库缺少最新正式版所需的 {table} 表"));
        }
    }
    Ok(())
}
...
fn part_messages(message: &MessageRow, part: &PartRow) -> Vec<Value> {
    ...
    match part_type {
        ...
        "tool" => {
            ...
            let mut value = message_value("tool", &text, &timestamp, "zcode_part", status, false);
...
mod tests {
    ...
    fn fixture() -> (tempfile::TempDir, Source) {
        ...
        connection
            .execute(
                "insert into session values (?1, null, ?2, ?3, ?4, ?5, ?6)",
                params![
                    "ses_main",
                    "/work/project",
                    "修复构建流程",
        ...
        connection
            .execute(
                "insert into session values (?1, ?2, ?3, ?4, ?5, ?6, ?7)",
                params![
                    "ses_child",
                    "ses_main",
                    "/work/project",
        ...
        let parts = [
            ...
            (
                "prt_05",
                "msg_assistant",
                json!({"type":"tool","callID":"call_ok","tool":"Bash","state":{"status":"completed","input":{"command":"pwd"},"output":"/work/project","time":{"start":1_787_184_003_000_i64,"end":1_787_184_004_000_i64}}}),
            ),
            (
                "prt_06",
                "msg_assistant",
    ...
    #[test]
    fn sqlite_格式聚合会话摘要并排除子代理() {
        let (_directory, source) = fixture();
        let parsed = parse_source(&source).unwrap();
        // subagent_child 会话被排除，只保留主会话
        ...
        assert_eq!(main.summary["model_provider"], "zhipu");
    ...
    fn 详情会映射文本_思考和工具结果() {
        ...
        assert_eq!(detail["raw_events"].as_array().unwrap().len(), 6);
    }
    ...
    fn 旧格式不会被当作最新_sqlite_格式读取() {
        ...
        let error = match parse_source(&source) {
            Ok(_) => panic!("旧格式不应被识别为最新 ZCode 数据库"),
            Err(error) => error,
        };
